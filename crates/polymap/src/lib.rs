...
impl PolyMap {
    pub fn new(width: usize, height: usize, poisson_radius: f64) -> Self {
        ...
        let voronoi = voronator::VoronoiDiagram::<voronator::delaunator::Point>::from_tuple(
            &(-1.0,-1.0), &(width as f64, height as f64), &centers
        ).expect("Failed to build voronoi diagram");

        let polygons:Vec<_> = voronoi.cells().iter().map(|poly| {
    ...
    pub fn polygon_at(&self, px: f32, py: f32) -> Option<CellId> {
        ...
        let point = quadtree_rs::point::Point { x: px.round() as u64, y : py.round() as u64 };
        let area = quadtree_rs::area::AreaBuilder::default().anchor(point).dimensions((1, 1)).build().unwrap();
    ...
    pub fn cells(&self) -> impl Iterator<Item=(CellId, &Cell)> {
        self.cells.iter().enumerate().map(|(id, cell)| (CellId(id), cell))
    }

    pub fn edges(&self) -> impl Iterator<Item=(EdgeId, &Edge)> {
        self.edges.iter().enumerate().map(|(id, edge)| (EdgeId(id), edge))
    }
    ...
    fn polygon_area(polygon: &Polygon<f64>) -> quadtree_rs::area::Area<u64> {
        ...
        let min_p = quadtree_rs::point::Point { x: min_x.ceil() as u64, y: min_y.ceil() as u64 };
