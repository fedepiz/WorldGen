    ...
    fn draw_cells(ctx: &mut Handle, poly_map:&PolyMap, shader:&impl MapShader, validation: &Validation) {
        ...
        let mut geometry = VertexBuffers::<Point, u16>::new();
        {
            ...
            for (id, cell) in poly_map.cells() {
                ...
                let points:Vec<_> = cell.polygon.exterior().points_iter().map(|p|
                     lyon::geom::point(p.x() as f32, poly_map.height as f32 - p.y() as f32)).collect();
                ...
                for triangle in geometry.indices.chunks(3) {
                    ...
                    ctx.draw_triangle(make_point(triangle[0]), make_point(triangle[1]), make_point(triangle[2]), color)
                }
            }
        }
    }
...
impl Validation {
    ...
    fn join(&mut self, other: Validation) {
        match self {
            ...
            Self::Invalid => {},
            Self::Partial(partial_invalidation) => {
                match other {
